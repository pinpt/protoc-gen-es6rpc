...
func methodParamsComment(desc *descriptor.MethodDescriptorProto, index int, types map[string]*descriptor.DescriptorProto, comments map[string]*descriptor.SourceCodeInfo_Location, indent string) h.HTML {
	...
	lines = append(lines, "@type {Object} request parameters")
	lines = append(lines, "@property {Object} headers request headers")
	lines = append(lines, "@property {string} authorization request authorization header")
	lines = append(lines, "@property {string} baseurl base url to prepend to the path")
	lines = append(lines, "@property {string} credentials authentication credentials mode: omit, same-origin, include")
...
func generate(in []byte) []byte {
	...
	for _, service := range services {
		buf.WriteString("export * from './" + service + "';\n")
	}
	// write out a file that will export all of our services
	resp.File = append(resp.File, &plugin.CodeGeneratorResponse_File{
		Name:    strp("main.js"),
		Content: strp(buf.String()),
	})
...
{{ range $i, $method := .service.Method }}
	 ...
	 */
	static {{.Name}}({{ buildArgsObject $method $types }} = {}, { headers = {}, authorization = '', baseurl = '', credentials = '' } = {}) {
		return new Promise((resolve, reject) => {
			...
			if (authorization) {
				...
				credentials = credentials || baseurl !== '' ? 'include' : 'same-origin';
			} else {
			...
			const url = baseurl + '{{$apiprefix}}/{{.Name}}';
			fetch(url, { method: 'POST', body: JSON.stringify(data), headers: reqHeaders, credentials }).then(resp => {
				if (resp.ok) {
					return resp.json();
				} else {
